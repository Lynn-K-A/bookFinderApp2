  ...
  fetch(endpoint)
    .then(async (response) => {
      try {
        ...
        console.log(
          `Oops.. something went wrong! check the endpoint at ${error}`
        );
      }
    })
    .then((jsonData) => {
...
//import { changeSearchBy } from './lib/eventHandlers.js';
...
window.onload = () => {
  ...
  globalThis.changeSearchBy = changeSearchBy;
  restoreDb();
  console.log("books", getVal("books"));
  renderResults(getVal("books"));
